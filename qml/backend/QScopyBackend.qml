...
Item {
    ...
    function getDaemonPath() {
        return "/home/lowell/.config/qscopy/bin/qscopy-daemon";
    }
    ...
    Process { id: copyProc; onExited: { if (backendRoot.closeOnCopy) Qt.quit(); } }
    ...
    function copyItem(id) {
        ...
        if (backendRoot.closeOnCopy) {
            let t = Qt.createQmlObject('import QtQuick; Timer { interval: 500; onTriggered: Qt.quit() }', backendRoot);
            t.start();
        }
    }
    ...
    function setGlassBlur(val) { backendRoot.glassBlur = val; silentProc.command = [getDaemonPath(), "config", "set", "glass_blur", val.toString()]; silentProc.running = false; silentProc.running = true; }
